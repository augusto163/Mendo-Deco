...
export async function GET() {
  try {
    ...
    // 2. Activos de Inversion
    const activosCount = await prisma.activoInversion.count();
    if (activosCount === 0) {
    ...
    // 3. Bobinas
    const bobinasCount = await prisma.bobinaFilamento.count();
    if (bobinasCount === 0) {
    ...
    // 4. Trabajos
    const trabajosCount = await prisma.trabajoProduccion.count();
    if (trabajosCount === 0) {
